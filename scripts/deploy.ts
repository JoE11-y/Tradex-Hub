...
for (const contract of contracts) {
  ...
  try {
    ...
    if (contract.packageName === "tradex-hub") {
      // tradex-hub has custom constructor: admin + attestor_pubkey + game_hub
      deployResult =
        await $`stellar contract deploy --wasm-hash ${wasmHash} --source-account ${adminSecret} --network ${NETWORK} -- --admin ${adminAddress} --attestor_pubkey ${attestorPubkeyHex} --game_hub ${mockGameHubId}`.text();
    } else {
      deployResult =
    ...
    // Set badge VK for tradex-hub if circuit is built
    if (contract.packageName === "tradex-hub") {
...
const deploymentInfo = {
  ...
  attestorPubkey: attestorPubkeyHex,
...
const tradexHubId = deployed["tradex-hub"] || "";
if (tradexHubId) {
  const backendEnvContent = `# Auto-generated by deploy script
# WARNING: Contains secret keys. Never commit!

ATTESTOR_SECRET_KEY=${attestorSecretHex}
