    ...
    <>
      ...
      <div className="md:hidden space-y-2 p-2">
        {openPositions.map((pos) => {
          ...
          return (
            <div key={pos.id} className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/30">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-white">{pos.asset}</span>
                  <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${pos.side === 'long' ? 'bg-green-900/50 text-green-400' : 'bg-red-900/50 text-red-400'}`}>
                ...
                <button
                  ...
                  disabled={closing}
                  className="px-3 py-1.5 text-xs font-semibold bg-slate-700 text-white rounded hover:bg-slate-600 transition-colors disabled:opacity-50 min-h-[36px]"
                >
                  {closing ? '...' : 'Close'}
                </button>
              </div>
              <div className="flex items-center justify-between text-xs">
                <div className="text-slate-400">
                  Entry: <span className="text-slate-300">${pos.entry_price.toFixed(pos.entry_price >= 100 ? 2 : 4)}</span>
                  <span className="ml-2">Size: <span className="text-slate-300">${pos.margin.toFixed(2)}</span></span>
                </div>
