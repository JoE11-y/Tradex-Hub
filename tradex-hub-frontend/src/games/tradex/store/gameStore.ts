...
import { getRouteNavigate } from '../routeNav';
...
export const useGameStore = create<GameState>()((set, get) => ({
  ...
  navigateTo: (page) => {
    ...
    // Also push react-router URL
    const { navigate, playerId } = getRouteNavigate();
    if (!navigate) return;

    const pageToPath: Record<PageName, string> = {
      lobby: '/',
      trading: `/games/trading/${playerId ?? 0}`,
      summary: `/games/summary/${playerId ?? 0}`,
      profile: `/games/profile/${playerId ?? 0}`,
      leaderboard: '/leaderboard',
      patterns: `/games/patterns/${playerId ?? 0}`,
      predictions: `/games/forecast/${playerId ?? 0}`,
    };
